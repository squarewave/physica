...
sim_entity_t* create_bogger(game_state_t* game_state, v2 position) {
    const f32 bogger_mass = 5.0f;
    const f32 bogger_orientation = 0.0f;

    sim_entity_t* bogger = create_block_entity(game_state,
                                               BOGGER,
                                               position,
                                               bogger_diagonal,
                                               bogger_mass,
...
UPDATE_FUNC(BOGGER) {
    ...
    bogger_state_t* state = &entity->bogger_state;
...
sim_entity_t* create_bogger_ball(game_state_t* game_state, v2 position, v2 direction) {
    const f32 bogger_ball_mass = 0.1f;
    const f32 bogger_ball_orientation = 0.0f;
    const f32 bogger_ball_speed = 20.0f;

    sim_entity_t* ball = create_fillet_block_entity(game_state,
                                                    BOGGER_BALL,
                                                    position,
